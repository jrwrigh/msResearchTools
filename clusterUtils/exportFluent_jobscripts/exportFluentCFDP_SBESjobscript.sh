...
cfdpFileName=$(basename $initDataFile .dat)
...
/file/export/cfd-post-compatible $cfdpFileName * () * ()
density
viscosity-lam
$velocities
$pressures
...
for node in `uniq $PBS_NODEFILE`
do
    ssh $node "cp -r $TMPDIR/${cfdpFileName}.cdat $PBS_O_WORKDIR"
done
