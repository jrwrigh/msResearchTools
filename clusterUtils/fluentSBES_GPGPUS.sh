#!/bin/bash
#PBS -N FluentSBES_GPGPUTest
#PBS -l select=3:ngpus=2:ncpus=40:mpiprocs=40:mem=32gb:phase=18b
...
# Organizing number of gpus per node
tot_nnodes=$(cat "$PBS_NODEFILE" | sort -u | wc -l)
tot_ngpus=$(qstat -f "$PBS_JOBID" | grep -Eo "ngpus = [0-9]" | grep -Eo "[0-9]")
ngpus_per_node=$(expr "$tot_ngpus" / "$tot_nnodes")
...
fluent_args="-t${tot_cpus} -gpgpu="$ngpu" $fluent_args -cnf=$PBS_NODEFILE"
